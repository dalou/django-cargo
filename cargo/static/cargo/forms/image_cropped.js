...
{
    if(!$container[0].cargo_image_cropped)
    {
        $container[0].cargo_image_cropped = true;
        self = this;
        self.$container = $container
        self.$input = self.$container.find('input[type=file]');
        self.$img = self.$container.find('.cargo-image_cropped-image img');
        ...
        if(self.$input.length && self.$input.attr('id').indexOf('__prefix__') == -1)
        {
            ...
            self.$container.on('click', '.cargo-image_cropped-change', function()
            {
                self.$input.click();
            });
            self.$container.on('click', '.cargo-image_cropped-remove', function()
            {
                self.$container.find('.cargo-image_cropped-image').removeClass('active');
                self.$container.find('.cargo-image_cropped-empty').addClass('active');
...
{
    self = this;
    self.$container.find('.cargo-image_cropped-image img').attr('src', src);
    self.$container.find('.cargo-image_cropped-image').addClass('active');
    self.$container.find('.cargo-image_cropped-empty').removeClass('active');
...
{
    $(document).on('mouseenter', '.cargo-image_cropped', function()
    {
        new fieldsBundleImageCropped($(this));
    });


    CLASS_VAR = 'cargo-croppedimage';
    ...
    APPLY = function($input, $input_cropped, src, $img, $container)
    {
        ...
        $container = $input_cropped[0].$cargo_cropped_image_container;
        if($container)
        {
    ...
    {
        // $img = $input_cropped.parent().find('img')
        var $container = $input_cropped[0].$cargo_cropped_image_container;
        if($container)
        {
        ...
        $input_cropped[0].$cargo_cropped_image_container = $container;
