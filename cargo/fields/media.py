...
class MediaFieldImage(FieldFile):
    ...
    def _get_html(self):
        if self.name and not self.type in self.field.authorized_types:
            return ''
        return """<img class="cargo-media" src="%s" />""" % self.url
...
class MediaFieldEmbed(object):
    ...
    #     if self.is_image():
    #         return """<img class="cargo-media" src="%s" />""" % self.name
    #     elif self.type in ['youtube']:
    #         return """<iframe class="cargo-media" src="%s" frameborder="0" allowfullscreen></iframe>""" % self.name
    #     elif self.type in ['soundcloud']:
    #         return """<iframe class="cargo-media" src="%s" scrolling="no" frameborder="0" allowfullscreen></iframe>""" % self.name
    #     else:
    #         return """<iframe class="cargo-media" src="%s" scrolling="no" frameborder="0" allowfullscreen></iframe>""" % self.name
    # html = property(_get_html)
