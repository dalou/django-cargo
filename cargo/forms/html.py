...
class HtmlInput(forms.Textarea):
    ...
    class Media:
        js = [
            cargo.settings.CARGO_TINYMCE_URL,
            'cargo/forms/html.js',
        ]
        css = {
            'all': ('cargo/forms/html.css', )
        }
    ...
    def get_tinymce_config(self, name, attrs):
        config = copy.deepcopy(cargo.settings.CARGO_TINYMCE_DEFAULT_CONFIG)
        ...
        config['language_url'] = settings.STATIC_URL + 'cargo/vendors/tinymce/langs/fr.js'
    ...
    def render(self, name, value, attrs=None):
        ...
        flatattrs['data-cargo-html_input'] = self.get_config_json(config)


        if self.inline:

            html = [u"""<div class="cargo-tinymce_inline">
